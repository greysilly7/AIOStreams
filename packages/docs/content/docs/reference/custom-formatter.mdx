...
| `::replace('a', 'b')`   | Replace every element equal to `a` with `b`            |
| `::unique`              | Drop repeated elements, keeping the first              |
...
`::keep`, `::remove`, `::replace` and `::unique` compare whole elements, ignoring case. `::keep` and `::remove` pair with the `{user.*}` lists, and `::unique` drops the repeats `::replace` can leave behind:

```
{stream.audioTags::keep({user.audioTags})::join(', ')}               ->  Atmos
{stream.languages::remove({user.languages})::join(', ')}             ->  German
{stream.visualTags::replace('HDR10+', 'HDR')::unique::join(' | ')}   ->  HDR | DV
```
...
`::each` renders its template once per item. For a track list, the track being rendered is available as `{track.*}` (see [Tracks](#tracks)). All other fields stay available too, and conditionals work inside it. An item that renders only whitespace is left out, but repeats are kept: two English tracks render `EN` twice, so add `::unique` before `::join` to show it once. Inside the template, quotes need escaping only outside `{…}`, as in conditional branches. `::each` cannot be nested inside another `::each`, and one render allows at most 256 items across all its `::each` calls.
