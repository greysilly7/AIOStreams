...
4. **RSDebrid** — a self-hosted download manager: AIOStreams hands it torrents and NZBs, it downloads to _your own_ servers, and plays back instantly once complete. See [RSDebrid](#rsdebrid).
...
| **RSDebrid**                           | Full download to your RSDebrid server first, then instant playback | Your own server | A self-hosted RSDebrid instance, indexers                                                                           |
...
<Callout type="info">
  ...
  **Easynews** work from any device, and **Stremio NNTP** works with just a
  provider if you're on a supported Stremio client. If you already self-host,
  **RSDebrid** adds torrent support to your Usenet setup from your own
  hardware.
</Callout>
...
[RSDebrid](https://rsdebrid.com) is a self-hosted download manager: you run it on your own servers, and it downloads **both torrents and Usenet releases** for you. AIOStreams treats it like a debrid service — with no per-download fees, because it runs on your hardware. Because it is your own instance, results it has already downloaded (torrents added by hash, or NZBs it has imported) count as cached; everything it hasn't seen yet is uncached — the download starts on demand, and you can use **Cache and Play** (under **Services → Built-in → Cache and Play**) to wait for it instead of restarting the stream.

**Setup:**

<Steps>
  <Step>Have RSDebrid running somewhere reachable, and note its API URL (and API
    key, if your instance requires one).</Step>
  <Step>
    In AIOStreams, go to **Services** and add the **RSDebrid** service. Enter
    the API URL — it is required. An optional **Public URL** can be set when
    RSDebrid is reachable at a different address from outside your network than
    from AIOStreams itself; it is what players are pointed at.
  </Step>
  <Step>
    Add Usenet addons — Newznab and friends for your own indexers. Torrent
    results from Torznab and other torrent addons play through RSDebrid the
    same way.
  </Step>
</Steps>

Your API key is only ever sent as a request header (a `Bearer` token) to your
RSDebrid instance — it is never embedded in URLs. Instances without an API key
work with the **API Key** field left empty.

<Callout type="warn">
  Cached/uncached status is per **your own** RSDebrid library: a release that
  another RSDebrid user has downloaded is still "uncached" for you until
  you've downloaded it yourself, so don't be surprised if the first play of
  popular content still has to download.
</Callout>

<Callout type="warn">
  **Cache and Play** here waits on RSDebrid — and your player may time out
  before a slow download finishes. A player that gives up after ~30 seconds
  (e.g. the Stremio iOS app) may need a manual stream restart once the
